...
// searchWorkflowCmd represents the search-workflow command
var searchWorkflowCmd = &cobra.Command{
	Use:   "search-workflow",
	Short: "Search for a workflow process definition by code",
	Long: `Search for a workflow process definition by providing the process code.
...
Examples:
  ...
  digit search-workflow --code MY_WORKFLOW_CODE --server http://localhost:8085`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Print response body
		fmt.Printf("Workflow search response:\n")
...
func init() {
	...
	// Add flags for search-workflow command
	searchWorkflowCmd.Flags().String("code", "", "Process code to search definition for (required)")
	searchWorkflowCmd.Flags().String("server", "", "Server URL (overrides config)")
	searchWorkflowCmd.Flags().String("jwt-token", "", "JWT token for authentication (overrides config)")
	
	// Mark required flags for search-workflow
